...
- `headline`：**<=20 字，硬限制，超过会被 CLI 拒绝。** 一个短句，不要用 `×` 或 `+` 拼接。必须是一个完整的表述。
  - **Style A — 成就型**：有数字、有对比
    - ✅ "用 AI 做产品的人"（9 字）
    - ✅ "72小时从零到上线"（8 字）
  - **Style B — 认证型**：有认证锚点
    - ✅ "AI 认证的全栈速通选手"（10 字）
    - ✅ "龙虾见证的产品架构师"（9 字）
  - ❌ "AI产品领袖 × 极致完美主义者"（× 拼接，太长，会被拒绝）
  - ❌ "一个人活成一支团队"（太 vague，没有具体信息）
  - ❌ "勤奋的开发者"（没有冲击力，换谁都行）
- `tagline`：一句让非技术人说"等一下"的话。**必须包含具体数字，<=60 字**
  - ✅ "3 只龙虾，127 天，从调研到上线全程 AI 协作"
  - ❌ "正在用AI重构自己的工作方式"（太虚，说了等于没说）
- `stats`：**严格固定 4 项，不可更改维度。** 每个有 `value`（纯数字，<=6 字符）和 `label`（<=6 字符）

  | # | 维度 | label | value 格式 | 数据来源 |
  |---|------|-------|-----------|---------|
  ...
  - ❌ `{"value": "6个/周", "label": "产品发现"}` — 这不是 stats 的内容，应该放 showcase
